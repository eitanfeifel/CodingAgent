...
import { initializePinecone, upsertEmbedding, queryEmbedding } from "./pineconeClient";
import { generateEmbedding } from "./llms/embeddings";
...
async function chunkAndEmbedFile(file: PRFile, index: any) {
  ...
  const chunkEmbeddings = await Promise.all(
    chunks.map(async (chunk, i) => {
      ...
      await upsertEmbedding(index, "default", `${file.filename}-${i}`, embedding, {
        filename: file.filename,
        chunk,
        startLine: 10,
        endLine: 20,
      });
      return { id: `${file.filename}-${i}`, embedding };
    })
  );
...
async function retrieveContextForFile(file: PRFile) {
  ...
  const matches = await queryEmbedding(index, "default", embedding, 5, { filename: file.filename });
